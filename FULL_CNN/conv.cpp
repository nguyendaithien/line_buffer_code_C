...
void print_line_buffer( int LineBuffer[IMAGE_WIDTH+2] ){
  for( int i = 0 ; i < IMAGE_WIDTH+2 ; i++ ) {
    printf("%d  ", LineBuffer[i]);
    }
    printf("\n");
  }
template<typename T, int K, int NUMBER_KERNEL_1>
static void convolution_1( int padding, int width, int hight ,hls::stream<T> &image,const T *kernel,hls::stream<T> &output) {
 int conv_size = K;
 hight = hight + 2*padding;
 width = width + 2*padding;
   
 T window_buffer[K][K];
 static T line_buffer_0[IMAGE_WIDTH+2];
 static T line_buffer_1[IMAGE_WIDTH+2];
 static T line_buffer_2[IMAGE_WIDTH+2];
...
#pragma HLS PIPELINE
    for( int j = 0 ; j < width; j++ ) { 
      ...
      if( i == 0 || j == 0 || i == IMAGE_HIGHT + 1 || j == IMAGE_WIDTH + 1){
	      line_buffer_2[j] = 0;
      }
      else {
...
template<typename T, int K, int NUMBER_FILTERS_1>
static void max_pooling(int width, int hight, hls::stream<T> &input, hls::stream<T> &output) {
    ...
    T line_buffer_0[NUMBER_FILTERS_1][IMAGE_WIDTH];
    T line_buffer_1[NUMBER_FILTERS_1][IMAGE_WIDTH];
    T window_buffer[NUMBER_FILTERS_1][K-1][K-1];

    for( int num_fil1 = 0 ; i < NUMBER_FILTERS_1; num_fil1++){
...
template<typename T, int NUM_FILTERS_1>   
void pool1(int width, int hight, hls::stream<data_t> &input, hls::stream<data_t> &output) {
...
template<typename T, int K,int NUMBER_FILTER_1, int NUMBER_FILTERS_2>
static void convolution_2( int padding, int width, int hight ,hls::stream<T> &image,const T *kernel,hls::stream<T> &output) {
 int conv_size = K;
 hight = hight + 2*padding;
 width = width + 2*padding;
 int count_conv2 = 0;  
 T window_buffer[NUM_FILTER_1][K][K];

 static T line_buffer_0[NUMBER_FILTERS_1][IMAGE_WIDTH+2];
 static T line_buffer_1[NUMBER_FILTERS_1][IMAGE_WIDTH+2];
 static T line_buffer_2[NUMBER_FILTERS_1][IMAGE_WIDTH+2];

 T data[NUMBER_FILTERS_2];
...
// push pixel to 1 line buffer
FILTER_1:for ( int num_fil1 = 0; num_fil1 < NUMBER_FILTERS_1; num_fil1 ++ ){
IMGH: for( int i = 0 ; i < hight ; i++ ) {
#pragma HLS DEPENDENCE variable=line_buffer_0 inter false
#pragma HLS DEPENDENCE variable=line_buffer_1 inter false
#pragma HLS DEPENDENCE variable=line_buffer_2 inter false
#pragma HLS PIPELINE
INGW:for( int j = 0 ; j < width; j++ ) { 
	    data[NUMBER_FILTERS_2] = {0,0,0,0,0,0,0,0};
FILTER_1:for ( int num_fil1 = 0; num_fil1 < NUMBER_FILTERS_1; num_fil1 ++ ){
      line_buffer_0[num_fil1][j] = line_buffer_1[num_fil1][j];
      line_buffer_1[num_fil1][j] = line_buffer_2[num_fil1][j];
      if( i == 0 || j == 0 || i == IMAGE_HIGHT + 1 || j == IMAGE_WIDTH + 1){
	      line_buffer_2[num_fil1][j] = 0;
      }
      else {
      line_buffer_2[num_fil1][j] = image.read();
    }
    }
     }
    print_line_buffer(line_buffer_0);
    ...
    if(i > 1){
	// initial window buffer
        window_buffer[num_fil1][0][0] = 0;
        window_buffer[num_fil1][1][0] = 0;
        window_buffer[num_fil1][2][0] = 0;
        window_buffer[num_fil1][0][1] = line_buffer_0[num_fil1][0];
        window_buffer[num_fil1][1][1] = line_buffer_1[num_fil1][0];
        window_buffer[num_fil1][2][1] = line_buffer_2[num_fil1][0];
        window_buffer[num_fil1][0][2] = line_buffer_0[num_fil1][1];
        window_buffer[num_fil1][1][2] = line_buffer_1[num_fil1][1];
        window_buffer[num_fil1][2][2] = line_buffer_2[num_fil1][1];
 // shift window to right
Shift_win_right:for( int b = 0 ; b < width - 2 ; b++) {
#pragma HLS DEPENDENCE variable=line_buffer_0 inter false
#pragma HLS DEPENDENCE variable=line_buffer_1 inter false
...
#pragma HLS PIPELINE
        for( int n = 0 ; n < 3; n++) {
          window_buffer[num_fil1][n][0] = window_buffer[num_fil1][n][1];
          window_buffer[num_fil1][n][1] = window_buffer[num_fil1][n][2];
          if( n == 0) {
            window_buffer[num_fil1][n][2] = line_buffer_0[num_fil1][b+2];
          }
	  if( n == 1) {
            window_buffer[num_fil1][n][2] = line_buffer_1[num_fil1][b+2];
          }
          if( n == 2) {
            window_buffer[num_fil1][n][2] = line_buffer_2[num_fil1][b+2];
          }
       }
// convolution caculate    
T sum = 0;
      for( int num_fil2 = 0; num_fil2 < NUMBER_KERNEL_2; num_fil2 ++) {
 COnv:for (int m = 0; m < K; ++m) {
#pragma HLS PIPELINE
#pragma HLS UNROLL
      for (int n = 0; n < K; ++n) {
         sum += window_buffer[num_fil1][m][n] * kernel[num_fil2*num_fil1*m*3+n];
        }
	 }
      }
         //sum += bias_conv1[num_filters];
...
void convolution_layer_1( int padding, int width, int hight,hls::stream<int> &image,hls::stream<int> &output) {
...
convolution<data_t>(padding, width, hight, image, kernel, output);
}

void conv2(int padding,int width, int hight, hls::stream<data_t> &src_conv1, hls::stream<data_t> &dst_conv1) {
    #pragma HLS DATAFLOW
...
{{0.2513255, 0.09718115, 0.18008718, 0.084324464, 0.25022158, -0.020569649, 0.57322866, 0.1851626, 0.29006967},
  ...
  {-0.009426449, -0.013661123, 0.06446785, 0.07858565, -0.025933357, 0.22884853, -0.074847765, 0.13717866, 0.009457402}},
    ...
    convolution_2<data_t>(padding, width, hight, src_conv1, filter2, dst_conv1);
...
void full(hls::stream<data_t> &full_in,hls::stream<data_t> &full_out){
   ...
   max_pooling(IMG_WIDTH,IMG_HEIGHT,conv1_out,full_out);
